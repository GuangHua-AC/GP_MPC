...
def main() -> None:
    ...
    if args.backend == "torch":
        ...
        model = TorchDynamicsModel.load(task_output_subdir("balance", "models") / "balance_nn_torch.pt", device=args.device)
    else:
        model = NNDynamicsModel.load(task_output_subdir("balance", "models") / "balance_nn.pt")
    ...
    out = task_output_subdir("balance", "mpc") / f"balance_external_push_{force_tag}N_{duration_ms}ms_T{t_tag}_Tp{tp_tag}_nn_mpc_{args.backend}.npz"
    np.savez(
        out,
        states=states_np,
        actions=actions_np,
    ...
    print("task=balance_external_push_nn_mpc")
