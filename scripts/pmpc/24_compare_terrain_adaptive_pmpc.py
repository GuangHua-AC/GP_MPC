...
def load_result(method: str, path: Path, notes: str) -> dict | None:
    ...
    n = min(len(states), len(actions), len(support_rolls), len(leg_adapt))
...
def main() -> None:
    known = newest("outputs/terrain/pmpc/tuning/H_chance200_guide50_*seed0.npz")
    adaptive = newest("outputs/terrain_adaptive/pmpc/terrain_adaptive_pmpc_*seed0*.npz")
    specs = [
        ("Known terrain GP-PMPC", known, "Known terrain reference, not adaptive."),
        ("Adaptive GP-PMPC", adaptive, "Blind adaptive wrapper with GP-PMPC."),
        ...
        ("No-adaptive terrain PD", OUTPUT_DIR / "terrain" / "pd" / "terrain_left_obstacle_v0p15_pd.npz", "Known-terrain PD/VMC baseline; uses terrain difference."),
    ]
    results = [r for method, path, notes in specs if path is not None and (r := load_result(method, path, notes)) is not None]
    if not results:
        print("no adaptive terrain results")
        return
    ...
    for res in results:
        ...
        axes[2].plot(t, states[:, 12], label=f"{label} leg_diff")
    ...
    labels = ["roll / rad", "support_roll / rad", "leg_diff / adapt / m", "theta / rad", "phi / rad", "x / m", "action_norm"]
    for ax, ylabel in zip(axes, labels):
        ax.set_ylabel(ylabel)
        ax.grid(True, alpha=0.3)
        ax.legend(loc="best", fontsize=7)
    axes[-1].set_xlabel("time / s")
    fig.suptitle("Terrain Adaptive GP-PMPC Compare")
    fig.tight_layout()
    out_fig = ADAPTIVE_DIR / "figures" / "terrain_adaptive_pmpc_compare.png"
    out_fig.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_fig, dpi=160)
    plt.close(fig)

    out_csv = ADAPTIVE_DIR / "metrics" / "terrain_adaptive_pmpc_compare.csv"
    ...
    print("method,steps,final_reason,max_abs_roll,max_abs_support_roll,max_abs_theta,max_abs_phi,max_abs_x,max_abs_leg_diff,mean_plan_time_sec,total_runtime_sec")
    for res in results:
        print(
            f"{res['method']},{res['steps']},{res['final_reason']},{res['max_abs_roll']:.5f},"
            f"{res['max_abs_support_roll']:.5f},{res['max_abs_theta']:.5f},{res['max_abs_phi']:.5f},"
            f"{res['max_abs_x']:.5f},{res['max_abs_leg_diff']:.5f},{res['mean_plan_time_sec']:.5f},"
            f"{res['total_runtime_sec']:.3f}"
        )
