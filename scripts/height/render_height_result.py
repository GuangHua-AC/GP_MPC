...
def load_result(npz_path: str):
    ...
    if len(l0s) < len(states):
        l0s = np.pad(l0s, (0, len(states) - len(l0s)), mode="edge")
    if len(l0_refs) < len(states):
        l0_refs = np.pad(l0_refs, (0, len(states) - len(l0_refs)), mode="edge")
    meta = {
    ...
    return states, actions, l0s, l0_refs, meta
...
class HeightRenderer:
    def __init__(self, states, actions, l0s, l0_refs, meta, params, poly_cls):
        self.states = states
        self.actions = actions
        self.l0s = l0s
        self.l0_refs = l0_refs
        self.meta = meta
        self.p = params
    ...
    def draw_five_bar_leg(self, body_center, rot_body, wheel_axis, side, idx):
        p = self.p
        wheel_center = wheel_axis + np.array([0.0, side * p.D / 2.0, 0.0])
        if idx > 0:
    ...
    def draw_robot(self, idx: int):
        p = self.p
        theta, _theta_dot, x, _x_dot, phi, _phi_dot = self.states[idx, :6]
        l0 = self.l0s[min(idx, len(self.l0s) - 1)]
        rot_body = rotation_matrices(yaw=0.0, roll=0.0, pitch=phi)
        wheel_axis = np.array([x, 0.0, p.R], dtype=float)
        ...
        for side in [1, -1]:
            self.draw_five_bar_leg(body_center, rot_body, wheel_axis, side, idx)
        self.ax3d.quiver(
            body_center[0],
            body_center[1],
            body_center[2] + 0.12,
        ...
        l0_ref = self.l0_refs[min(idx, len(self.l0_refs) - 1)]
        self.ax3d.quiver(
            body_center[0],
            body_center[1],
            body_center[2] + 0.10,
    ...
    def draw_info_panel(self, idx: int):
        self.ax_info.clear()
        self.ax_info.set_title("Balance + Height Result", fontsize=12)
        self.ax_info.axis("off")
        theta, theta_dot, x, x_dot, phi, phi_dot = self.states[idx, :6]
        l0 = self.l0s[min(idx, len(self.l0s) - 1)]
        l0_ref = self.l0_refs[min(idx, len(self.l0_refs) - 1)]
        action = self.actions[min(idx, len(self.actions) - 1)]
        action = np.pad(action, (0, max(0, 6 - len(action))), mode="constant")
        T, Tp, _Tyaw, _Froll, Fheight = action[:5]
        lines = [
            ...
            f"theta_d  = {theta_dot:+.4f} rad/s",
            f"x        = {x:+.4f} m",
            f"x_dot    = {x_dot:+.4f} m/s",
            f"phi      = {phi:+.4f} rad  ({np.rad2deg(phi):+.2f} deg)",
            f"phi_dot  = {phi_dot:+.4f} rad/s",
            f"L0       = {l0:+.4f} m",
            f"L0_ref   = {l0_ref:+.4f} m",
            f"L0_err   = {l0 - l0_ref:+.4f} m",
            "",
            "Control action:",
            f"T        = {T:+.4f}",
            f"Tp       = {Tp:+.4f}",
            f"Fheight  = {Fheight:+.4f}",
            "",
            "Black links = closed-chain five-bar visual links",
            "Green arrow = current height tracking error",
        ]
        ...
        inset.plot(t, self.l0s[: len(t)], label="L0")
        inset.plot(t, self.l0_refs[: len(t)], "--", label="L0_ref")
        inset.plot(t, self.states[:, 0], label="theta rad")
        inset.plot(t, self.states[:, 4], label="phi rad")
    ...
    def update(self, idx: int):
        ...
        self.ax3d.set_title("Balance + Variable Height Five-Bar Animation")
...
def main() -> None:
    ...
    states, actions, l0s, l0_refs, meta = load_result(args.npz)
    params = RenderParams(dt=float(meta["dt"]))
    fps = args.fps if args.fps else output_fps(params.dt, max(1, args.stride), args.speed)
    renderer = HeightRenderer(states, actions, l0s, l0_refs, meta, params, Poly3DCollection)
