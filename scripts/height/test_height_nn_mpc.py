...
def main() -> None:
    ...
    if args.backend == "torch":
        ...
        model = TorchDynamicsModel.load(task_output_subdir("height", "models") / "height_nn_torch.pt", device=args.device)
    else:
        model = NNDynamicsModel.load(task_output_subdir("height", "models") / "height_nn.pt")
    ...
    out = task_output_subdir("height", "mpc") / height_filename("nn_mpc", args.mode, args.low, args.high, args.v_ref, backend=args.backend)
