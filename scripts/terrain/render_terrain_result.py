...
def load_result(npz_path: str):
    ...
    meta = {
        ...
        "terrain_mode": str(scalar(data, "terrain_mode", "left_obstacle")),
        "v_ref": float(scalar(data, "v_ref", 0.15)),
    }
...
class TerrainRenderer:
    def __init__(self, states, actions, meta, params, terrain_params, poly_cls):
        self.states = states
    ...
    def draw_five_bar_leg(self, body_center, rot_body, wheel_center, side, idx):
        ...
        draw_cylinder(self.ax3d, wheel_center, p.R, p.wheel_width, np.eye(3), "#111111", spin_angle=self.wheel_spin)
    ...
    def draw_robot(self, idx: int):
        p = self.p
        theta, _theta_dot, x, _x_dot, phi, _phi_dot, _yaw, _yaw_dot, roll, _roll_dot, alpha = self.states[idx, :11]
        l0 = 0.30 + 0.10 * np.sin(alpha)
        left_h, right_h = self.ground_at(x)
        avg_h = 0.5 * (left_h + right_h)
        rot_body = rotation_matrices(0.0, roll, phi)
        wheel_axis = np.array([x, 0.0, avg_h + p.R], dtype=float)
        body_center = wheel_axis + np.array([p.R * np.sin(theta), 0.0, l0], dtype=float)
        draw_box(self.ax3d, body_center, p.body_dims, rot_body, "#dddddd", self.poly_cls)
        left_wheel = np.array([x, +p.D / 2.0, left_h + p.R], dtype=float)
        right_wheel = np.array([x, -p.D / 2.0, right_h + p.R], dtype=float)
        ...
        self.ax3d.quiver(body_center[0], body_center[1], body_center[2] + 0.12, 0.35, 0.0, 0.0, linewidth=2.0, arrow_length_ratio=0.2, color="tab:blue")
    ...
    def draw_terrain(self, cx: float):
        xs = np.linspace(cx - 0.8, cx + 0.8, 80)
        left = np.array([self.ground_at(x)[0] for x in xs])
        right = np.array([self.ground_at(x)[1] for x in xs])
        self.ax3d.plot(xs, np.full_like(xs, +self.p.D / 2.0), left, color="tab:green", lw=3.0)
        self.ax3d.plot(xs, np.full_like(xs, -self.p.D / 2.0), right, color="sienna", lw=3.0)
        for y, zline in [(+self.p.D / 2.0, left), (-self.p.D / 2.0, right)]:
            self.ax3d.plot(xs, np.full_like(xs, y), np.zeros_like(xs), color="0.8", lw=0.8, alpha=0.6)
            for x, z in zip(xs[::8], zline[::8]):
                self.ax3d.plot([x, x], [y, y], [0, z], color="0.75", lw=0.6)

    def draw_info_panel(self, idx: int):
        self.ax_info.clear()
        self.ax_info.set_title("Terrain Adaptive Result", fontsize=12)
        self.ax_info.axis("off")
        s = self.states[idx]
        theta, theta_dot, x, x_dot, phi, phi_dot, _yaw, _yaw_dot, roll, roll_dot, alpha, alpha_dot, leg_diff, leg_diff_dot = s
        left_h, right_h = self.ground_at(x)
        ...
        lines = [
            ...
            f"Terrain: {self.meta.get('terrain_mode', 'unknown')}",
            f"v_ref = {self.meta.get('v_ref', 0.0):+.3f} m/s",
            ...
            f"phi        = {phi:+.4f} rad",
            f"roll       = {roll:+.4f} rad ({np.rad2deg(roll):+.2f} deg)",
            f"leg_diff   = {leg_diff:+.4f} m",
            f"terrain_d  = {terrain_diff:+.4f} m",
            f"support_r  = {support_roll:+.4f} rad ({np.rad2deg(support_roll):+.2f} deg)",
            "",
            "Control action:",
            f"T          = {action[0]:+.4f}",
            f"Tp         = {action[1]:+.4f}",
            f"F_roll     = {action[3]:+.4f}",
            f"Fheight    = {action[4]:+.4f}",
            f"leg_cmd    = {action[5]:+.4f}",
        ]
        ...
        for st in self.states:
            lh, rh = self.ground_at(st[2])
            terrain_diffs.append(lh - rh)
            support_rolls.append(np.arctan2(lh - rh + st[12], self.p.D))
        inset.plot(t, np.asarray(terrain_diffs), label="terrain diff m")
        inset.plot(t, self.states[:, 12], label="leg_diff m")
        inset.plot(t, np.rad2deg(self.states[:, 8]), label="roll deg")
        inset.plot(t, np.rad2deg(support_rolls), label="support roll deg")
    ...
    def update(self, idx: int):
        self.ax3d.clear()
        cx = self.states[idx, 2]
        view_range = 0.8
        self.ax3d.set_xlim(cx - view_range, cx + view_range)
        self.ax3d.set_ylim(-view_range, view_range)
        self.ax3d.set_zlim(0.0, 0.8)
        self.ax3d.set_xlabel("World X / m")
        self.ax3d.set_ylabel("World Y / m")
        self.ax3d.set_zlabel("Z / m")
        self.ax3d.set_title("Terrain Adaptive Five-Bar Animation")
        self.ax3d.set_proj_type("ortho")
        self.ax3d.set_box_aspect((1.0, 1.0, 0.75))
        self.ax3d.view_init(elev=24, azim=-58)
        xx, yy = np.meshgrid(np.linspace(cx - view_range, cx + view_range, 8), np.linspace(-view_range, view_range, 8))
        self.ax3d.plot_wireframe(xx, yy, np.zeros_like(xx), color="gray", alpha=0.22, linewidth=0.5)
        self.draw_terrain(cx)
        self.ax3d.plot(self.states[: idx + 1, 2], np.zeros(idx + 1), np.zeros(idx + 1), color="0.55", lw=1.2, alpha=0.55)
        self.draw_robot(idx)
...
def main() -> None:
    ...
    renderer = TerrainRenderer(states, actions, meta, params, terrain_params, Poly3DCollection)
